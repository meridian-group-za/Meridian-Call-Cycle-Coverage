...
# Manager / Head of Syndicated filters in sync as the roster changes
# (new reps/merchandisers added, people reassigned). Runs build_hierarchy.py,
# and if it produced a real change, commits and pushes straight to main so
# the live GitHub Pages dashboard picks it up on its next load.
#
...
try {
    ...
    $diff = git status --porcelain hierarchy.json
    if (-not $diff) {
...
current Call Cycle Master roster via scripts/build_hierarchy.py.
